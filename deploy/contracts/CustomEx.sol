...
contract CustomDex {
    ...
    constructor() {
        for (uint i = 0; i < tokens.length; i++) {
            CustomToken tokenInstance = new CustomToken(tokens[i], tokens[i]);
            tokenInstanceMap[tokens[i]] = tokenInstance;
        }
    }
    ...
    function swapTokenToEth(string memory tokenName, uint256 _amount) public returns (uint256) {
        ...
        require(address(this).balance >= ethToBeTransferred, "Oops! Dex is running low on balance");
    ...
    function swapTokenToToken(string memory srcTokenName, string memory destTokenName, uint256 _amount) public {
        require(tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this), _amount), "Transfer failed");
        require(tokenInstanceMap[destTokenName].transfer(msg.sender, _amount), "Transfer failed");
